...
{
    /// <summary>
    /// Toobit market socket
    /// </summary>
    internal class ToobitWebsocketPublic : IWebsocketPublic
    {

        private ToobitMarket m_oMarket;

        private const int MAX_SUBSCRIPTIONS = 200; // Max subscriptions per socket
        private ConcurrentDictionary<int, ToobitSocketSingle> m_aSockets = new ConcurrentDictionary<int, ToobitSocketSingle>();
        private IWebsocketDataManager m_oDataManager;


        public ToobitWebsocketPublic(ToobitMarket oMarket)
        {
            m_oMarket = oMarket;
        ...
        public async Task<IWebsocketSubscription?> Subscribe(IFuturesSymbol oSymbol, WsMessageType eSubscriptionType)
        {
            ...
            m_aSockets[nNext] = new ToobitSocketSingle(this);
