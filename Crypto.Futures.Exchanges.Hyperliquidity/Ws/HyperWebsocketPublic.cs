...
{
    internal class HyperWebsocketPublic : IWebsocketPublic
    {

        private HyperLiquidityMarket m_oMarket;
        private IWebsocketDataManager m_oDataManager;
        private const int MAX_SUBSCRIPTIONS = 100;

        private ConcurrentDictionary<int, HyperWebsocketSingle> m_aSockets = new ConcurrentDictionary<int, HyperWebsocketSingle>();

        public HyperWebsocketPublic(HyperLiquidityMarket oMarket)
        {
            m_oMarket = oMarket;
        ...
        public async Task<IWebsocketSubscription?> Subscribe(IFuturesSymbol oSymbol, WsMessageType eSubscriptionType)
        {
            ...
            m_aSockets[nNext] = new HyperWebsocketSingle(this);
