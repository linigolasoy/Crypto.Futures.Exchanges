...
{
    internal class BitgetWebsocketPublic: IWebsocketPublic
    {
        private BitgetMarket m_oMarket;

        private const int MAX_SUBSCRIPTIONS = 200; // Max subscriptions per socket

        private ConcurrentDictionary<int, BitgetSocketSingle> m_aSockets = new ConcurrentDictionary<int, BitgetSocketSingle>();
        private IWebsocketDataManager m_oDataManager;
        public BitgetWebsocketPublic(BitgetMarket oMarket)
        {
            m_oMarket = oMarket;
        ...
        public async Task<IWebsocketSubscription?> Subscribe(IFuturesSymbol oSymbol, WsMessageType eSubscriptionType)
        {
            ...
            m_aSockets[nNext] = new BitgetSocketSingle(this);
